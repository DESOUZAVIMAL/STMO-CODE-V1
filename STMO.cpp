...
// ============================================================
// Run 002: Global adaptive config variables
// Set at runtime based on N after genData() is called.
// All stages and main loop use these instead of #define defaults.
// ============================================================
int   g_maxIter  = 2500;   // default, overridden adaptively
float g_endTime  = 120.0f;  // default, overridden adaptively
int   g_kStag    = 50;      // default, overridden adaptively
int   g_minCount = 3;       // default, overridden adaptively
...
void printDiagnosticReport(const DiagCounters& diag,
    ...
    if (diag.iterBestImproved == diag.totalIterations) {
        ...
        printf("║   Suggestion: increase g_endTime in config.h  ║\n");
    ...
    if (diag.stage2TotalRepairs == 0) {
        ...
        printf("║   Suggestion: decrease g_minCount in config.h ║\n");
    } else {
...
int main() {
    ...
    genData();
    // ── Run 002: Set adaptive config based on problem size ──────────
    if (N_Order <= 50) {
        g_maxIter  = MAX_ITER_SMALL;   // 10000
        g_kStag    = K_STAG_SMALL;     // 100
        g_minCount = MIN_COUNT_SMALL;   // 3
        g_endTime  = 120.0f;
    } else if (N_Order >= 150) {
        g_maxIter  = 2500;             // MAX_ITER_LARGE
        g_kStag    = K_STAG_LARGE;     // 50
        g_minCount = MIN_COUNT_LARGE;   // 1
        g_endTime  = END_TIME_LARGE;    // 180
    } else {
        g_maxIter  = 2500;             // default
        g_kStag    = 50;               // default
        g_minCount = 3;                // default
        g_endTime  = 120.0f;           // default
    }
    printf("[Adaptive] N=%d -> MaxIter=%d EndTime=%.0fs KStag=%d MinCount=%d\n",
           N_Order, g_maxIter, g_endTime, g_kStag, g_minCount);
    // ────────────────────────────────────────────────────────────────

    printf("[Config] P=%d NC=%d DF=%.2f g_kStag=%d E=%d\n\n",
           P, NC, DF, g_kStag, E_SIZE);
