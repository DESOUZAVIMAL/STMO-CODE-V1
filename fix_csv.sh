...
# Run after run_all_experiments.sh finishes.
# Usage: bash fix_csv.sh

LOGS="./results/logs"
OUT_CSV="./results/results.csv"

if [ ! -d "$LOGS" ]; then
    echo "ERROR: $LOGS not found. Run experiments first."
    exit 1
fi
...
for f in "$LOGS"/out_N*_M*_S*.txt; do
    ...
    # Parse N, M, Seed from filename: out_N150_M9_S30.txt
    BASE=$(basename "$f" .txt)          # out_N150_M9_S30
    N=$(echo "$BASE"  | sed 's/out_N\([0-9]*\)_M.*/\1/')
    M=$(echo "$BASE"  | sed 's/.*_M\([0-9]*\)_S.*/\1/')
    S=$(echo "$BASE"  | sed 's/.*_S\([0-9]*\)/\1/')

    # Parse values — -m1 ensures only first match (avoids header collisions)
...
# Sort by N, M, Seed for clean ordering
